...
def run(root):
    out = root / 'structure_association_validation_02'
    if out.exists():
    ...
    excluded = read('structure_mapping_audit_02/foldseek_chain_reconciliation.json')['unmatched']
    ...
    associations = read('structure_mapping_audit_02/qualified_chain_associations.json')
    ...
    for q, members in associations.items():
        ...
        for r in members:
            ...
            header = r['foldseek_identifier']
            if '_' in header and header != r['file'][:4] + '_' + r['chain']:
                problems.append('explicit_chain_identifier:' + q)
    ...
    census = read('structure_mapping_audit_02/availability_census.json')
    ...
    names = ['verify_structure_associations_v2.py', 'structure_assets_01/source_manifest.json',
             'structure_mapping_audit_02/qualified_chain_associations.json', 'structure_mapping_audit_02/primary_representatives.json',
