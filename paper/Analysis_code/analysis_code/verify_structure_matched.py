...
def run(root):
    result = root / 'structure_matched_01'; out = root / 'structure_matched_validation_01'
    if out.exists(): raise FileExistsError(out)
    read = lambda n: json.loads((root / n).read_text())
    failures = []; maxdiff = 0.; fits = 0
    for n, sha in read('structure_matched_01/input_manifest.json').items():
    ...
    blocks = read('multiaxis_split_01/outer_inner_blocks.json'); receipts = read('structure_matched_01/block_receipts.json')
    ...
    ev = read('structure_matched_01/evaluation.json')
