...
def run(root):
    out = root / 'structure_matched_01'
    if out.exists(): raise FileExistsError(out)
    read = lambda name: json.loads((root / name).read_text())
    if read('structure_association_validation_01/INDEPENDENT_QC.json')['status'] != 'PASS':
    ...
    selected = read('structure_mapping_audit_01/primary_representatives.json'); available = set(selected)
    census = {r['sequence_sha256']: r for r in read('structure_mapping_audit_01/availability_census.json')}
    ...
    names = ['run_structure_matched.py', 'STRUCTURE_MATCHED_ANALYSIS_V1.md', 'run_main_baselines.py', 'dataset_02/core_edges.json',
             'dataset_02/core_reactions.json', 'multiaxis_split_01/outer_inner_blocks.json', 'multiaxis_split_01/axis_assignments.json',
             'structure_mapping_audit_01/primary_representatives.json', 'structure_mapping_audit_01/availability_census.json',
             'structure_association_validation_01/INDEPENDENT_QC.json', 'foldseek_raw_01/all_vs_all.tsv',
