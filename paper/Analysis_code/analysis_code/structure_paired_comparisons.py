...
def run(root):
    result = root / 'structure_matched_01'; out = root / 'structure_paired_01'
    if out.exists(): raise FileExistsError(out)
    if json.loads((root / 'structure_matched_validation_01/INDEPENDENT_QC.json').read_text())['status'] != 'PASS': raise ValueError('Metric gate')
    ...
    names = ['structure_paired_comparisons.py', 'paired_expert_comparisons.py', 'STRUCTURE_MATCHED_ANALYSIS_V1.md',
             'structure_matched_01/per_query_metrics.jsonl', 'structure_matched_validation_01/INDEPENDENT_QC.json']
