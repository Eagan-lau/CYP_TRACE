...
output=root/'protein_control_validation_01/REPLICATE_000_FITTING_QC.json'
...
blocks=read('multiaxis_split_01/outer_inner_blocks.json'); receipts=read('protein_controls_01/replicate_000/block_receipts.json')
axes=read('multiaxis_split_01/axis_assignments.json')
failures=[]; max_pred_error=0.; nested_count=0; outer_count=0; lambda_checks=0


def reconstruct(block,declared,assignment):
    ...
    train=np.where(Y.sum(1)>0)[0]
    if train.tolist()!=assignment['recipient_indices'] or set(train)!=set(assignment['donor_indices']): raise ValueError('Bad training assignment')
    Y[train]=Y[np.array(assignment['donor_indices'])].copy()
    target=(Y[train]/Y[train].sum(1,keepdims=True))@K
...
for bn,block in enumerate(blocks):
    ...
    info=read(f'protein_controls_01/replicate_000/block_{bn:03d}_inner_manifest.json')
    saved=np.load(result/f'block_{bn:03d}_inner_predictions.npz',allow_pickle=False)
    fits={r['inner_number']:r for r in receipts[bn]['inner_fits']}; rows=info['rows']; truths=info['targets']
    ...
    for j,indices in grouped.items():
        ...
        qs,target,lp,delta=reconstruct(inner,fits[j],fits[j]['assignment']); nested_count+=1
    ...
    if choice['reason']=='zero_boundary_optimum' and derivative < -1e-8: failures.append('zero_boundary_derivative:'+str(bn))
    ...
    lambda_checks+=1
    declared=receipts[bn]['outer_fit']
    qs,target,lp,delta=reconstruct(block,declared,receipts[bn]['outer_assignment']); outer_count+=1
    scorefile=np.load(result/f'block_{bn:03d}_scores.npz',allow_pickle=False)
    expected=lp[None,:]+value*delta
    error=float(np.max(np.abs(expected-scorefile['train_profile_permuted_residual']))); max_pred_error=max(max_pred_error,error)
...
receipt={'status':'PASS' if not failures else 'FAIL','created_utc':now(),'failures':failures,
    ...
    'scope':'Replicate 000 only: every nested prediction, target/group identity and selected coefficient optimum; other 98 refits are not independently reconstructed here',
