...
root = Path(__file__).resolve().parent; result = root / 'structure_paired_02'; out = root / 'structure_paired_validation_02'
...
orig = [json.loads(s) for s in (root / 'structure_matched_02/per_query_metrics.jsonl').read_text().splitlines()]
...
for bn, members in byblock.items():
    d = np.load(root / 'structure_matched_02' / f'block_{bn:03d}_scores.npz', allow_pickle=False)
    for row in members:
