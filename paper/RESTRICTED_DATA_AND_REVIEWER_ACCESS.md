...
acceptance receipt are supplied under `upstream/`; the listed raw files are
obtained separately from their providers.
...
|---|---|---|
| P450Rdb v2.0 reaction and protein files | Mixed-source evidence qualification and general-CYP development core | Downloaded on 29 July 2026 from the [official download page](https://www.cellknowledge.com.cn/p450rdb_v2/download.html). The study inputs contain 3,821 reaction records and 1,012 protein records; filenames, sizes and SHA-256 checksums are recorded in `upstream/core_input_manifest.json`. Raw files are obtained from the provider and are not redistributed in this package; derived assertion decisions and query metrics are supplied. |
...
The download date identifies the P450Rdb inputs used in this study. A later
provider download checked in September contained updated records; exact
reconstruction uses the files identified by the manifest. The official link
is an acquisition route, not a permanent archive of the July files.
