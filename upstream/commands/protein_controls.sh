...
FS="$FOLDSEEK"
mkdir -p logs
# Execute once for every task index from 0 through 98.
CYPTRACE_TASK_INDEX="${2:?Supply the task index as the second argument (0-98)}"
[[ "$CYPTRACE_TASK_INDEX" =~ ^[0-9]+$ ]] || exit 2
(( CYPTRACE_TASK_INDEX >= 0 && CYPTRACE_TASK_INDEX <= 98 )) || exit 2
set -euo pipefail
export OPENBLAS_NUM_THREADS=4 OMP_NUM_THREADS=4 MKL_NUM_THREADS=4
"$PYTHON" run_protein_controls.py --replicate "$CYPTRACE_TASK_INDEX"
